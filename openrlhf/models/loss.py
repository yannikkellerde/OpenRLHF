...
class PolicyLoss(nn.Module):
    ...
    def __init__(self, clip_eps: float = 0.2, second_bound=None) -> None:
        super().__init__()
        # Second bound is non-standard in ppo
        # It avoids problems with extreme ratios
        # It should not be needed technically, but well.
        # Set to something > 100 if at all
    ...
    def forward(
        ...
        if self.second_bound is not None:
            surr1 = ratio.clamp(1 - self.second_bound, 1 + self.second_bound) * advantages
        else:
